...
class Action {
    ...
    enQueue(actionFn = () => { }, actionInfo = {
        ...
        if (action) {
            ...
            debug$2 && console.info('【 ActionManager log 】交互入队', actionInfo);
        }
        // 对交互队列进行重新排序
        this.queue.sort((a, b) => {
        ...
        debug$2 && console.info('【 ActionManager log 】交互排序', this.queue);
    }
    /**
     * unQueue 交互出队
     */
    unQueue() {
        ...
        if (!action) {
            debug$2 && console.info('【 ActionManager log 】交互已全部结束');
            // 执行 allDone 监听事件回调
    ...
    off(event = 'action', namespace = '') {
        if (!event) {
            debug$2 && console.info('【 ActionManager log 】移除所有事件监听函数');
     ...
     */
    clear() {
        debug$2 && console.info('【 ActionManager log 】清空交互队列');
        // 遍历交互队列
...
class Dialog {
    ...
    show(dialogInfo = { name: '', data: {} }) {
        debug$1 && console.info(`【 DialogManager log 】${dialogInfo.name} 被打开`);
        // 记录当前弹窗数据
    ...
    close(payload = {}) {
        ...
        debug$1 && console.info(`【 DialogManager log 】${dialogInfo.name} 被关闭`);
        // 执行 close 监听事件
    ...
    off(event = 'show', namespace = '') {
        if (!event) {
            debug$1 && console.info('【 DialogManager log 】移除所有事件监听函数');
...
var index = new EventManager();

exports.ActionManager = index$2;
exports.DialogManager = index$1;
exports.EventManager = index;
